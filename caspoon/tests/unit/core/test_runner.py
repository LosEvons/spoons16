...
class TestReconRunner:
    ...
    def test_runner_logs_step_execution(self, sample_binary: str, caplog) -> None:
        ...
        for step in runner.steps:
            assert any(step.name in record.message for record in caplog.records), \
    ...
    def test_runner_logs_errors(self, sample_binary: str, caplog) -> None:
        ...
        assert any("Error in step" in record.message for record in caplog.records), \
    ...
    def test_report_enrichment_only(self, sample_binary: str) -> None:
        ...
        assert "pre-existing" in result.strings, \
    ...
    def test_runner_with_multiple_binaries(self, test_binaries_dir) -> None:
        ...
        for binary in binaries:
            ...
            assert isinstance(report, ExecutableReport), \
                f"Should produce report for {binary.name}"
            assert report.path == str(binary), \
                f"Path should match for {binary.name}"
            # Should have some data populated
            assert report.file_type != "", \
                f"Should populate file_type for {binary.name}"
