...
class TestFullPipeline:
    ...
    def test_binary_characteristics(
        ...
        # Verify basic info
        assert report.arch in ["x86_64", "x86"], \
            f"Should detect architecture for {binary_name}"
        assert report.bits in [32, 64], \
            f"Should detect bit width for {binary_name}"
        assert "ELF" in report.file_type, \
            f"Should detect ELF file type for {binary_name}"
        
        # Verify stripped status
        assert report.stripped == expected_stripped, \
        ...
        if report.protections and report.protections.relro != "checksec_not_found":
            assert report.protections.pie == expected_pie, \
    ...
    def test_multiple_runs_same_binary(
        self, runner: ReconRunner, sample_binary: str
    ) -> None:
    ...
    def test_pipeline_error_recovery(
        ...
        assert isinstance(report, ExecutableReport), \
            "Should return report even with errors"
        assert report.path == "/nonexistent/path", \
            "Path should be preserved even with errors"
