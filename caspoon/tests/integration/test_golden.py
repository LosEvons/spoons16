...
class TestGoldenOutputs:
    ...
    def _normalize_report(self, report_dict: dict) -> dict:
        ...
        if "raw_backend_data" in normalized:
            ...
            normalized["raw_backend_data"] = {
                k: v
                for k, v in normalized.get("raw_backend_data", {}).items()
    ...
    def test_golden_test_hello_x64(
        ...
        if not golden_path.exists():
            pytest.skip(
                f"Golden file not found: {golden_path}. " "Run with --update-golden to create."
            )
        ...
        assert current_output["path"] == expected_output["path"], "Binary path should match"
        assert current_output["arch"] == expected_output["arch"], "Architecture should match"
        assert current_output["bits"] == expected_output["bits"], "Bit width should match"
        assert (
            current_output["stripped"] == expected_output["stripped"]
        ), "Stripped status should match"

        # Protections should match
        if expected_output.get("protections"):
            assert (
                current_output["protections"] == expected_output["protections"]
    ...
    def test_golden_test_with_pie(
        ...
        assert (
            current_output["protections"]["pie"] == expected_output["protections"]["pie"]
        ), "PIE protection should match"
        assert (
            current_output["protections"]["canary"] == expected_output["protections"]["canary"]
        ), "Stack canary should match"
        assert (
            current_output["protections"]["nx"] == expected_output["protections"]["nx"]
        ), "NX protection should match"
        assert (
            current_output["protections"]["relro"] == expected_output["protections"]["relro"]
