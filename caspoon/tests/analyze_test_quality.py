...
def print_report(filepath: Path, metrics: dict):
    ...
    if metrics["test_functions"] > 0:
        ...
        print(
            f"Type hints: {metrics['tests_with_type_hints']}/{metrics['test_functions']} ({type_hint_pct:.1f}%)"
        )

        docstring_pct = (metrics["docstrings"] / metrics["test_functions"]) * 100
        print(
            f"Docstrings: {metrics['docstrings']}/{metrics['test_functions']} ({docstring_pct:.1f}%)"
        )

    if metrics["assertions"] > 0:
        assertion_msg_pct = (metrics["assertions_with_messages"] / metrics["assertions"]) * 100
        print(
            f"Assertion messages: {metrics['assertions_with_messages']}/{metrics['assertions']} ({assertion_msg_pct:.1f}%)"
        )
...
def main():
    ...
    if sys.argv[1] == "--analyze":
        ...
        if total_metrics["test_functions"] > 0:
            type_hint_pct = (
                total_metrics["tests_with_type_hints"] / total_metrics["test_functions"]
            ) * 100
            print(f"Type hint coverage: {type_hint_pct:.1f}%")

        if total_metrics["assertions"] > 0:
            msg_pct = (
                total_metrics["assertions_with_messages"] / total_metrics["assertions"]
            ) * 100
