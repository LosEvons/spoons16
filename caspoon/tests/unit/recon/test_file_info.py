...
class TestFileInfoRecon:
    ...
    def test_analyze_system_binary(
        self, recon: FileInfoRecon, sample_binary: str
    ) -> None:
    ...
    def test_nonexistent_file(self, recon: FileInfoRecon) -> None:
        ...
        assert "Error: File not found" in result.file_type, \
            "Should note file not found error"
    ...
    def test_directory_not_file(self, recon: FileInfoRecon, tmp_path: Path) -> None:
        ...
        assert "Error: Not a file" in result.file_type, \
            "Should note that path is not a file"
    ...
    def test_report_enrichment(
        ...
        assert result.arch != "" or result.file_type != "", \
            "Should populate at least one field"
    ...
    @pytest.mark.parametrize("arch_string,expected_arch", [
        ("x86-64", "x86_64"),
        ("x86_64", "x86_64"),
        ("amd64", "x86_64"),
        ("i386", "x86"),
